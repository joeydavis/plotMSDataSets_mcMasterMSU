...
def calculateStatsFile(data_dictionary, protein_set, numerator, denominator, normalization=1.0):
    ...
    for uid in data_dictionary:
        ...
        u = data_dictionary[uid]["ampu"]
        l = data_dictionary[uid]["ampl"]
        s = data_dictionary[uid]["amps"]
        valNum = 0
        for i in numerator:
            if i == "ampu":
                valNum = valNum + u
...
def determineNormalization(dataByProtein, proteinName):
    return numpy.median(dataByProtein[proteinName]["vals"])
...
def plotStatsFile(proteins, dataByProtein, name, offset=0.0, markerSize=40, color='#e31a1c'):
    ...
    pylab.yticks([0,0.25,0.50,0.75,1.0,1.25], size=15)
    pylab.ylim(0, 1.25)
...
if __name__ == '__main__':
    ...
    path = '/home/jhdavis/scripts/python/plotMSDataSets_mcMasterMSU/McMasterMSUCsvs/'
